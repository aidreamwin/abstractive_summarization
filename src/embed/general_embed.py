...
class GeneralEmbed(object):
    ...
    def merge_embed_from_file(self):
        ...
        with open("embed_dict.bin","wb") as f:
    ...
    def search_best_ids(self, content=""):
        ...
        # if sorted_list[0][1] >= 0.96:
        #     return sorted_list

        # # 对匹配到的label取前排数据再次匹配keyword
        # keyword_list = sorted_list[:3]
        # id_list = []
        ...
        # sec_list = []
        # for k,v in self.data_embed.items():
        #     if k not in id_list:continue
        #     d = self.consin(vec,v[4])
        #     d_s = (k,d)
        #     sec_list.append(d_s)

        # sorted_list = sorted(sec_list, key=lambda dd: dd[1],reverse=True)
    ...
    def get_keyword_for_null(self,sql_queue):
        ...
        self.data_embed = {}
        with open("embed_dict.bin", "rb") as f:
        ...
        for data in datas:
            ...
            str_key = "`keyword_general_embed_2` (`id`, `aid`, `ref_score`, `ref_keyword`, `content`, `ref_content`, `ref_id`)"
    ...
    def test(self):
        x = ["中国国家航天局发布的消息，长征十一号运载火箭此次发射的七颗卫星包含发射技术试验卫星捕风一号A、B星及五颗商业卫星"]
        vs = self.bert.get_embedding(x)
        i = self.bert.get_embedding(["你不能保护你自己的子模块名字在一个新版本的python中。"])[0]
        for v in vs:
            d = self.consin(i,v)
            print(d)
