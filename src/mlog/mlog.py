...
class Logger:
  def __init__(self, path = './general.log',clevel = logging.DEBUG,Flevel = logging.DEBUG):
    ...
    fh = logging.handlers.RotatingFileHandler(path, maxBytes=200*1024, backupCount=1000)
...
mlog = Logger(path=path_name,clevel=logging.DEBUG,Flevel=logging.INFO).getInstance()